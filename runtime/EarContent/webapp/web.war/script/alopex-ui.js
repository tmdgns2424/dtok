/*! Alopex UI - v2.2.34.5 - 2015-03-10
* http://ui.alopex.io
		...
		_blurHandler: function(e) {
		  ...
		  if (el.options.pickertype === 'daily' || el.options.pickertype === 'weekly') {
		    ...
		    if (!$.alopex.util.isValid(value)) {
		      ...
		      if (yearStartIdx > 0) {
		        date = new Date(value.substr(4,4), Number(value.substr(2,2))-1, value.substr(0,2));
		      } else {
		        date = new Date(value.substr(0,4), Number(value.substr(4,2))-1, value.substr(6,2));
		      }
		    } else {
		      if (yearStartIdx > 0) {
		        date = new Date(value.split(seperator)[2], Number(value.split(seperator)[1])-1, value.split(seperator)[0]);
		      } else {
		        date = new Date(value.split(seperator)[0], Number(value.split(seperator)[1])-1, value.split(seperator)[2]);
		      }
		    }
		    ...
		    if (!$.alopex.util.isValid(value)) {
		      ...
		      if (yearStartIdx > 0) {
		        date = new Date(value.substr(2,4), Number(value.substr(0,2))-1);
		      } else {
		        date = new Date(value.substr(0,4), Number(value.substr(4,2))-1);
		      }
		    } else {
		      if (yearStartIdx > 0) {
     ...
     */
		...
		_initialise: function() {
			...
			var subheader01 = this._render('div', {
				className: ((this.pickertype === 'daily' || this.pickertype === 'weekly') ? 'af-subHeader-year' : 'af-subHeader-year-wide')
			});
...
//		    } else if (!$.alopex.util.isValid(seperator)) {
//		      if (yearStartIdx > 0) {
//		        date = new Date(datestr.substr(4,4), Number(datestr.substr(2,2))-1, datestr.substr(0,2));
//		      } else {
//		        date = new Date(datestr.substr(0,4), Number(datestr.substr(4,2))-1, datestr.substr(6,2));
//		      }
//		    } else {
//		      if (yearStartIdx > 0) {
//		        date = new Date(datestr.split(seperator)[2], Number(datestr.split(seperator)[1])-1, datestr.split(seperator)[0]);
//		      } else {
//		        date = new Date(datestr.split(seperator)[0], Number(datestr.split(seperator)[1])-1, datestr.split(seperator)[2]);
//		      }
//		    }
//		  } else if (el.options.pickertype === 'monthly') {
//		    if (!$.alopex.util.isValid(datestr)) {
...
//		    } else if (!$.alopex.util.isValid(seperator)) {
//		      if (yearStartIdx > 0) {
//		        date = new Date(datestr.substr(2,4), Number(datestr.substr(0,2))-1);
//		      } else {
//		        date = new Date(datestr.substr(0,4), Number(datestr.substr(4,2))-1);
//		      }
//		    } else {
//		      if (yearStartIdx > 0) {
//		        date = new Date(datestr.split(seperator)[1], Number(datestr.split(seperator)[0])-1);
//		      } else {
//		        date = new Date(datestr.split(seperator)[0], Number(datestr.split(seperator)[1])-1);
//		      }
//		    }
//		  }
//		  return date;
//		},
		...
		_startEventHandler: function(date, datestr, certainDatesName, certainDatesInfo, e) {
		  ...
		  if(el.options.autodisable) {
		    if (el.options.pickertype == 'weekly') {
		      $(el._start).val(datestr['startdate']);
		      $(el._end).val(datestr['enddate']);
		    } else {
...
(function($) {
	...
	$.alopex.widget.pagination = $.alopex.inherit($.alopex.widget.object, {
		...
		setters: ['pagination', 'setTotalPage', 'setMaxPage', 'setSelectedPage', 'setCurrentPage', 'setDataLength', 'setPerPage', 'setPageLink'],
...
//			this.addPressHighlight(el);
		...
		init: function(el, options) {
			...
			el.pagelink = el.maxpage = parseInt($el.attr('data-maxpage'), 10) || parseInt($el.attr('data-pagelink'), 10) || el.maxpage;
		...
		_addButtonEventHandler: function(el) {
		  var $el = $(el);
			...
			for ( var i = 0; i < buttons.length; i++) {
				...
				$(buttons[i]).attr('data-type', 'button').button().on('click', handler);
			}
		},
...
//			$el.on('keyup', $.alopex.widget.spinner._keyupHandler);
		...
		_keydownHandler: function(e){
			var el = e.currentTarget;
			var $wrapper = $(el).closest('.af-spinner-wrapper');
			if($wrapper.hasClass('af-disabled')) return;
			if(e.which === 38) { // up
				$.alopex.widget.spinner.valueUp(el);
...
//					}
		...
		_addDisabledStyle: function(el, index) {
			...
			if (el.options.removebutton) {
			  $(el.removebuttons).eq(index).addClass('af-disabled');
			}
		},
		...
		_removeDisabledStyle: function(el, index) {
			...
			if (el.options.removebutton) {
        $(el.removebuttons).eq(index).removeClass('af-disabled');
      }
		},
...
!function($) {
	...
	function closeChildren() {
		if ($.alopex.popup.children && $.alopex.popup.children.length > 0) {
			for (var i = 0; i < $.alopex.popup.children.length; i++) {
				if ($.alopex.popup.children[i] && $.alopex.popup.children[i].close) {
					$.alopex.popup.children[i].close();
				}
			}
		}
	}
	...
	$(window).on('beforeunload', closeChildren);
...
//				}
				$.alopex.popup.result = data; //TODO TODO TODO
				window.close();
			} else {
				// 다이얼로그를 찾아서 닫기
...
!function($) {
	...
	$.alopex.request = function(id, option) {
		...
		if($.alopex.util.isValid(request.platform)) { // platform data setup
		  $.extend(true, request.data, request[request.platform].interface); 
		}
