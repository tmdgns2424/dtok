...
+(function($) {
	...
	function _renderValue(render, value, data, mapping) {
		...
		rendered += _renderValue.plugin[type || "general"](render, value, copied, mapping);
		return rendered;
	}
	...
	_renderValue.plugin["general"] = function(render, value, data, mapping) {
		...
		if (typeof render == "string") {
			...
			rendered += render(value, data, mapping);
...
//		tag.child = _escapeHTML(tag.child);
	...
	_renderValue.plugin["textarea"] = function(render, value, data, mapping) {
		...
		var tag = {
			tag: "textarea",
			attr: {
				name: mapping.key//_generateUniqueId()
			},
			child: value
		};
		tag.attr = $.extend(tag.attr, $.isPlainObject(render.attr) ? render.attr : null);
	...
	_renderValue.plugin["text"] = function(render, value, data, mapping) {
		...
		rendered.push('name="' + (mapping.key || _generateUniqueId()) + '" ');
		if (render.styleclass) {
	...
	_renderValue.plugin["select"] = function(render, value, data, mapping) {
		...
		rendered.push(mapping.key || _generateUniqueId(), '" ');
	...
	_renderValue.plugin["radio"] = function(render, value, data, mapping) {
		var rendered = [];
		var name = mapping.key || _generateUniqueId();
		var rules = render.rule;
		if (typeof rules == "function") {
			rules = rules(value, data, mapping);
		}
...
//			}
		...
		if (self.option.columnMapping) {
			...
			$.each(self.option.columnMapping, function(idx, mapping) {
				...
				if(mapping.hasOwnProperty('key') && mapping.hasOwnProperty('value') &&
					_valid(mapping.key) && _valid(mapping.value)) {
...
//		}
	...
	AlopexGrid.prototype._processDefaultSorting = function() {
		...
		for ( var i in self.option.columnMapping) {
			...
			var sorting = _readMappingProp(mapping, 'sorting');
			//if (mapping.sorting) {
			if(sorting) {
				...
				if (sorting == "desc" || sorting == "asc") {
					...
					if (Number(mapping.columnIndex) === Number(self.option.defaultSorting.columnIndex)) {
...
//					fixedColgroup.children('col').map(function(idx,elem){if(idx>self.state.fixupto) return elem;}).remove();
	...
	AlopexGrid.prototype._wrapperHeightRefresh = function() {
		...
		/*
		 * hmap.push(tableheaderHeight); -> self.state.tableheaderHeight
		 hmap.push(self.$wrapper.outerHeight()-self.$wrapper.height());
		 hmap.push((self.state.rowHeight)*rowcount);
		 hmap.push(self.$scroller.height()-self.$scroller.prop('clientHeight'));
		 * */
		...
		//var tableHeight = (option.height || option["max-height"] ? Number($r.css("height").split('px')[0]) : 0)
		var tableHeight = "";
		if(typeof self.state.userHeight === "string" && self.state.userHeight.indexOf('row')>=0) {
			...
			tableHeight = (option.height || option["max-height"] || $r.height() || 0) - ($title.data('alopex-grid-visible') ? $title.height() : 0) -
				($pager.data('alopex-grid-visible') ? $pager.outerHeight() : 0);
			var minus1 = Number($wrapper.css('border-top-width').split('px')[0]);
			var minus2 = Number($wrapper.css('border-bottom-width').split('px')[0]);
			...
			if ($wrapper.css('box-sizing') === 'border-box') { //box-sizing : border-box 적용시 border가 높이 계산시 먹힘.
				var rht = Number($r.css('border-top-width').split('px')[0]);
				var rhb = Number($r.css('border-bottom-width').split('px')[0]);
	...
	AlopexGrid.prototype._columnResizeStart = function(e, column) {
		...
		var columnIndex = column !== undefined ? column : $target.parentsUntil($scroller, '.cell').eq(0).attr('data-alopexgrid-columnindex');
...
//				if (cm.width) {
		...
		function resizeUpHandler(e) {
			...
			if(e.data.mapping.width && String(e.data.mapping.width).indexOf('%')>=0) {
				...
				var m1width = e.data.mapping.width ? Number(e.data.mapping.width.split("px")[0]) : null;
				var m2width = e.data.nextmapping && e.data.nextmapping.width ? Number(e.data.nextmapping.width.split("px")[0]) : null;
				if (m2width === 0)
					m2width = null;
				if (m1width === null) {
//					m1width = e.data.target.width(); // target 넘기는 부분 주석처리해서 에러발생. 
	...
	function _getMappingByQuery(columnMapping,query) {
		for(var ci in columnMapping) {
			...
			if($.isArray(mapping.multi)) {
				var r = _getMappingByQuery(mapping.multi, query);
				if(r) return r;
			}
		}
		return false;
	...
	AlopexGrid.prototype._cellEditUpdate = function(cell, dataid, key, e) {
		...
		if (key === undefined || key === null || typeof key === "number") {
			...
			columnIndex = Number(columnIndex);
			mapping = _getMappingByQuery(self.option.columMapping, {columnIndex:columnIndex});
		} else if (typeof key === "object" && key.hasOwnProperty('columnIndex')) {
			mapping = key;
		} else if (typeof key === "string") {
			mapping = _getMappingByQuery(self.option.columnMapping, {key:key});
		}
	...
	AlopexGrid.prototype._cellRender = function(data, mapping, option) {
		...
		if (mapping.numberingColumn) {
			...
			if(self.option.useTabindexOnEditable) {
				var tabindex = data._index.data * 1000 + mapping.columnIndex;
			...
			for(var mi in mapping.multi) {
				newbody.push('<tr class="row bodyrow" data-alopexgrid-dataid="',data._index.id,'" data-alopexgrid-dataindex="',data._index.data,'">');
				newbody.push(self._cellRender(data, $.extend({},mapping.multi[mi],{columnIndex:mapping.columnIndex}), option));
				newbody.push('</tr>');
			}
...
//			lastcolumn = columnIndex;
	...
	var _footerRenderer = {
		"sum" : function(param){
			...
			return _addCommas(Math.round(sum*m)/m);
		},
...
//			$row.replaceWith(_convertAlopex.call(self, newrow));
	...
	AlopexGrid.prototype.dataAdd = function(data, query) {
		...
		//    console.log('addedfrom',addedFrom);
		//    console.log('addlength',addLength);
		//    console.log('previously rendered', self.state.rendered)
		//    console.log('rowpadding?',self.option.rowPadding)
...
//			}
	...
	AlopexGrid.prototype._refreshEditableCell = function(dataIdIndex,$row){
		var self = this;
		var data = null;
		...
		if(!$cells.length) {
			...
			$cells.each(function(idx,cell){
				self._cellEditUpdate(cell, data._index.id, cell.getAttribute('data-alopexgrid-key'));
			});
		}
