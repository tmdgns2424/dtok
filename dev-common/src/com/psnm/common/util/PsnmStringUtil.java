...
/**
	...
	/**
	 * 원본 문자열에 포함된 대체지정 텍스트를 맵에 지정된 값으로 변경하여 반환
	 * - 원본문자열 : 대체지정 텍스트 : {0}, {1}, ...
	 * - 값저장맵  : 대체할 텍스트 키 : arg0, arg1, ...
	 * 
	 * @param message
	 * @param argMap
	 * @return
	 */
	public static String replaceArgs(String message, Map<String,Object> argMap) {
		if ( StringUtils.isBlank(message) || null==argMap || argMap.isEmpty() ) {
			return message;
		}
		...
		for (int i=0; i<10; i++) {
			if ( tmp.indexOf("{"+i+"}") >= 0 ) {
				String s = "\\{"+i+"\\}";
				String t = (String)argMap.get("{"+i+"}");
				if (null!=t)
					tmp = tmp.replaceAll(s, t);
			}
		}
		return tmp;
	}
	...
	public static void main(String[] args) throws Exception {
		String message = "11111111111 {0} 2222222222 {1} 333333333333 {2}";
		Map<String,Object> argMap = new java.util.HashMap<String, Object>();
		argMap.put("{0}", "아규먼트0");
		argMap.put("{1}", "아규먼트1");
		argMap.put("{2}", "아규먼트2");
		String sms = replaceArgs(message, argMap);
		System.out.println("- message = [" + message + "], sms = [" + sms + "]");
	}
}