...
//
    System.out.println("- =============================== 엑셀다운로드(전체) : 검색조건으로 조회후 다운로드");
    ...
    try {
        ...
        for(int x=0; x<metaInfoListCount; x++) {
            ...
            IRecordSet recordSet = dataSet.getRecordSet(gridname); //넥스코어 레코드셋 객체
            int colsSize = null==colsList ? 0 : colsList.size();
            int dataSize = null==recordSet ? 0 : recordSet.getRecordCount(); //레코드개수(LOOP회수)
            System.out.println("- RecordSet[" + gridname + "] dataSize = " + dataSize + "");
            ...
            for (int i = 0; i < dataSize; i++) {
                ...
                IRecord dataMap = recordSet.getRecord(i);
                ...
                for (int j = 0; j < colsSize; j++) { //데이터의 칼럼은 '칼럼정보' 기준으로 추가함
                    ...
                    String _val   = "";
                    try {
                        _val = dataMap.get(_key);
                    }
                    catch(Exception e) {
                        if (i<1) { //1회만 출력
                            System.err.println("<psnmExcelAll> <ERROR> " + e.getMessage() + " <column key> " + _key + "");
                            System.err.println("<psnmExcelAll> <ERROR> (수정방법) 필터칼럼에 이 칼럼(" + _key + ")의 인덱스 포함.");
                        }
                        _val   = "";
                    }
                    ...
                    cell = row.createCell(j);
                    cell.setCellValue( new HSSFRichTextString(_val) );
        ...
        System.err.println("<psnmExcelAll> <ERROR> " + e.getMessage() + "");
        e.printStackTrace();
    }
...
public static ArrayList<Map<String, String>> getParamData(String rawStrData) throws Exception {
    if (null==rawStrData) {
        return new ArrayList<Map<String, String>>();
    }

    ArrayList<Map<String, String>> dataList = null;
    String decodedStrData = null;
    ...
    try {
        ...
        decodedStrData = URLDecoder.decode(rawStrData, "UTF-8");

        dataList = gson.fromJson(decodedStrData, mapListType);
        ...
        System.err.println("<ERROR> <psnmExcelAll#getParamData> " + e.getMessage() + "");
        System.err.println("<ERROR> <psnmExcelAll#getParamData> rawStrData : [" + rawStrData + "]");
        System.err.println("<ERROR> <psnmExcelAll#getParamData> decodedData : [" + decodedStrData + "]");
        throw e;
    }
